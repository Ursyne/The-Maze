...
func _physics_process(delta):
	...
	if Input.is_action_pressed("ui_left") and not Input.is_action_pressed("ui_right") && isAttacking == false:
		motion.x = -SPEED
		motion.y = 0
		get_node("Character").set_flip_h(true)
		$Character.play("run")
		Facing_Direction = "Horizontal"
	elif Input.is_action_pressed("ui_right") and not Input.is_action_pressed("ui_left") && isAttacking == false:
		motion.x =  SPEED
		motion.y = 0
		get_node("Character").set_flip_h(false)
		$Character.play("run")
		Facing_Direction = "Horizontal"
	elif Input.is_action_pressed("ui_down") and not Input.is_action_pressed("ui_up")&& isAttacking == false:
		motion.y = SPEED
		motion.x =  0
		$Character.play("run")
		Facing_Direction = "Vertical"
	elif Input.is_action_pressed("ui_up") and not Input.is_action_pressed("ui_down") && isAttacking == false:
		motion.y = -SPEED
		motion.x = 0
		$Character.play("run")
		Facing_Direction = "Vertical"
